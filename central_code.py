"""
central_code.py — Constant-SGD as a Bayesian Sampler
=====================================================
Runs all experiments: SGD-sampler comparisons (constant, diagonal, full preconditioner)
plus gradient-noise diagnostics across datasets and models.

Supports:
    - Linear regression (exact MAP + Hessian)
    - Logistic regression (exact MAP + Hessian)
    - Neural networks via PyTorch (MAP via Adam, Hessian via autograd)

Key design:
    - ModelInterface / Dataset dataclass
    - run_sgd: n_runs × n_steps_per_run, keep last n_samples_per_run
    - KL from Lyapunov covariance vs empirical sample covariance
"""
...
@dataclass
class Dataset:
		"""Container returned by any dataset loader."""
		name: str
		X:    np.ndarray   # (N, D)
		y:    np.ndarray   # (N,)
...
class ModelInterface(ABC):
		...
		Loss convention:
				L(θ) = task_loss(θ) + (λ/2) ‖θ‖²
		where λ is NOT divided by N — consistent with the paper's prior scaling.
		"""

		@abstractmethod
		def init_params(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
				"""Return initial θ (zeros or warm start)."""
		...
		def stoch_grad(
				...
				"""Mini-batch stochastic gradient (unbiased)."""
		...
		def hessian(
				...
				"""Full Hessian ∇²L(θ), shape (D, D)."""

		def find_map(
				self, X: np.ndarray, y: np.ndarray, lam: float, **kw,
		) -> np.ndarray:
				"""Default: L-BFGS-B. Override for closed-form solutions."""
				from scipy.optimize import minimize
				res = minimize(
						lambda th: self.loss_and_grad(th, X, y, lam),
						self.init_params(X, y), jac=True, method="L-BFGS-B",
						options={"maxiter": kw.get("max_iter", 10_000),
										 "gtol":    kw.get("tol", 1e-10)},
				)
				return res.x
...
class LinearModel(ModelInterface):
    ...
    If bias=True, θ = [w, b] ∈ R^{D+1} and the bias b is NOT regularised.
    Closed-form MAP and exact Hessian.
    """
    def __init__(self, bias: bool = False):
        self.bias = bias

    def _aug(self, X: np.ndarray) -> np.ndarray:
        """Append a column of ones if bias=True."""
        if not self.bias:
            return X
        return np.concatenate([X, np.ones((len(X), 1))], axis=1)

    def _lam_vec(self, D_aug: int, lam: float) -> np.ndarray:
        """Regularisation vector: λ for weights, 0 for bias."""
        v = np.full(D_aug, lam)
        if self.bias:
            v[-1] = 0.0
        return v
...
class LogisticModel(ModelInterface):
    ...
    Binary logistic regression:
        L(θ) = -mean[y log σ + (1-y) log(1-σ)] + (λ/2) ‖θ‖²

    If bias=True, θ = [w, b] ∈ R^{D+1} and the bias b is NOT regularised.
    Exact Hessian via weighted outer product.
    """
    def __init__(self, bias: bool = False):
        self.bias = bias
...
class NeuralNetModel(ModelInterface):
    """
    Arbitrary PyTorch neural network wrapped in the ModelInterface.

    Parameters
    ----------
    net_factory  : callable() → torch.nn.Module, no required args
    task         : 'regression' | 'binary_classification' | 'multiclass'
    adam_lr      : Adam learning rate for MAP
    adam_steps   : number of Adam optimisation steps
    hessian_approx : 'full' — exact via torch.autograd (expensive)

    For 'multiclass': net output shape must be (batch, K) raw logits.
    Cross-entropy loss is applied directly (no softmax in net).
    """
    ...
    def _task_loss(self, net, X_t, y_t):
        ...
        if self.task == "regression":
            ...
            # y_t: (N,) long tensor of class indices
    ...
    def hessian(self, theta, X, y, lam):
        ...
        H_torch = hessian(loss_fn, th_flat)
    ...
    def predict(self, theta: np.ndarray, X: np.ndarray) -> np.ndarray:
        """Returns class probabilities for multiclass, else raw output."""
...
def load_adult(normalize: bool = True) -> Dataset:
    """
    Adult census income: N=48842, D=14 (after encoding), binary.
    Target: income >50K (1) or ≤50K (0).
    Mixes continuous and ordinal features — genuinely heterogeneous.
    """
    ...
    # Keep only numeric + ordinal columns (drop high-cardinality categoricals)
    numeric_cols = ["age", "education-num", "capital-gain", "capital-loss", "hours-per-week", "fnlwgt"]
    # Binary-encode low-cardinality categoricals
...
def load_digits_dataset(
    normalize:  bool = True,
    binary:     bool = False,   # True → digits 0 vs 1 only (P=64)
    n_pca:      int  = None,    # if set, reduce to n_pca PCA components
) -> Dataset:
    """
    sklearn's Optical Recognition of Handwritten Digits.
      N=1797, D=64 (8×8 pixels, values 0–16), K=10 classes.

    Options:
      binary=True  → keep only classes 0 and 1 (N≈360, binary task)
      n_pca=k      → project to top-k PCA components before returning
                     (reduces D for tractable Hessian/BBT)
    """
...
def load_custom(name: str, X: np.ndarray, y: np.ndarray,
								normalize: bool = True) -> Dataset:
		"""Wrap any numpy arrays into a Dataset."""
		return Dataset(name, _preprocess(X, normalize), y)


# ══════════════════════════════════════════════════════════════════════
# 5.  GRADIENT NOISE COVARIANCE  BBT = S · Cov[ĝ_S]
# ══════════════════════════════════════════════════════════════════════
...
def estimate_noise_cov(
    ...
    """
    BB^T = S * Cov(stochastic gradients) at theta.

    If n_mc is None: exact estimate over all N samples.
    If n_mc is set: Monte Carlo estimate using n_mc minibatches of size S (faster for large N).
    """
    N  = len(dataset.y)
    P  = model.nb_params
    F  = np.zeros((P, P))
...
def estimate_BBT_eigenvalues(
		...
		model:   ModelInterface,
		S:       int,
		k:       int = None,      # None = all, else top-k via randomized SVD
		batch:   int = 256,       # vectorized gradient batch size
) -> np.ndarray:
		"""
		Eigenvalues of BBT without ever forming the P×P matrix.

		Two regimes:
			N < P  → dual trick: form N×N Gram matrix GGᵀ  (cheap if N small)
			N >> P → randomized SVD of G directly           (cheap if k << P)

		Cost: O(N × P) to collect gradients, O(min(N,P)²) for eigenvalues.
		"""
		N, P = len(dataset.y), model.nb_params
		c    = (N - S) / (N - 1.0)

		# ── Step 1: collect gradient matrix G ∈ R^{N×P} in batches ──────
		G  = np.empty((N, P), dtype=np.float32)   # float32 saves memory
		idx = 0
		for start in tqdm(range(0, N, batch), desc="Collecting gradients"):
				...
				for i in range(b):
						n = start + i
						G[idx] = model.stoch_grad(
								theta,
								dataset.X[n:n+1], dataset.y[n:n+1],
								N=1, lam=0.0,
						)
						idx += 1

		g0 = G.mean(axis=0)   # (P,)  full gradient / N

		# ── Step 2: eigenvalues ───────────────────────────────────────────
		if k is not None:
				# ── Randomized SVD: top-k eigenvalues only ────────────────────
				# σ_i(G)² = λ_i(GᵀG) = λ_i(BBT) × N/c  (up to mean correction)
				from sklearn.utils.extmath import randomized_svd
				G_centered = G - g0[None, :]             # center for mean correction
				_, svals, _ = randomized_svd(
						G_centered, n_components=k,
						n_iter=4, random_state=0,
				)
				eigs = c * svals**2 / N                  # (k,) top eigenvalues of BBT

		elif N < P:
				# ── Dual trick: form N×N matrix instead of P×P ────────────────
				# BBT = c/N GᵀG - c g0 g0ᵀ
				# nonzero eigs of GᵀG = nonzero eigs of GGᵀ (× same factor)
				G_centered = G - g0[None, :]             # subtract mean: handles rank-1 term
				gram       = (G_centered @ G_centered.T) # (N, N)
				eigs_gram  = np.linalg.eigvalsh(gram)    # (N,)
				eigs       = np.maximum(c * eigs_gram / N, 0.0)   # rescale → BBT eigenvalues

		else:
				# ── Direct: N >> P, form P×P directly (original but vectorized) ─
		...
		total = eigs.sum()
		if total > 0:
				var_explained = np.cumsum(eigs) / total
				rank_99       = int(np.searchsorted(var_explained, 0.99)) + 1
		else:
				rank_99       = 0

		print(f"  BBT eigenvalues: max={eigs.max():.3e} rank(99% var)={rank_99}/{len(eigs)}")
		return eigs, g0   # descending

# ══════════════════════════════════════════════════════════════════════
# 6.  OU STATIONARY COVARIANCE   AΣ + ΣAᵀ = Q
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
# 7.  KL DIVERGENCE  KL(q ‖ posterior)  Eq. 16
# ══════════════════════════════════════════════════════════════════════
...
def optimal_H_fullrank(S: int, N: int, BBT: np.ndarray, eps_jitter: float = 1e-4) -> np.ndarray:
		"""Ridge-regularized inversion for the full-rank preconditioner."""
		D = BBT.shape[0]
...
def run_sgd(
		...
		S: int,
		lam: float,
		# step specification — exactly one of eps or H
		eps: float = 1.0,
		H: Optional[np.ndarray] = None,
		# sampling schedule
		n_runs:            int = 1,        # independent chains from θ*
		n_steps_per_run:   int = 50_000,   # SGD steps per chain
		n_samples_per_run: int = 20_000,   # last steps to keep per chain
		grad_clip: Optional[float] = None,
		log_every: int = 5_000,
) -> Optional[np.ndarray]:
		"""
		Collect constant-SGD samples.

		Step rule:
				H provided  →  θ ← θ - H @ ĝ_S          (ε absorbed into H)
				H = None    →  θ ← θ - ε · ĝ_S

		H must already satisfy max_eig(H @ A) < 2 (use make_stable() first).
		Returns array of shape (n_runs × n_samples_per_run, D), or None on failure.
		"""
		N      = len(dataset.y)
		...
		for run in tqdm(range(n_runs), desc="Runs", position=0):
				...
				for step in pbar:
						...
						# gradient clipping — prevents one bad mini-batch from exploding θ
						...
						if not np.all(np.isfinite(theta)):
								print(f"    !! run {run+1}: diverged at step {step} — skipping")
								ok = False
								break

						if log_every and step > 0 and step % log_every == 0:
									drift = np.linalg.norm(theta - theta_star)
									pbar.set_postfix({"|θ-θ*|": f"{drift:.4f}"})
...
# ══════════════════════════════════════════════════════════════════════
# 10.  POSTERIOR SAMPLER  (Laplace approximation)
# ══════════════════════════════════════════════════════════════════════

def sample_posterior(
		theta_star: np.ndarray, A: np.ndarray, N: int,
		n_samples: int = 5_000, seed: int = 42,
) -> np.ndarray:
		"""Draw i.i.d. samples from N(θ*, (NA)⁻¹) via Cholesky."""
...
@dataclass
class SGDConfig:
		"""Sampling configuration shared across all SGD variants."""
		n_runs:            int = 1
...
def run_experiment(
		...
		"""
		Full constant-SGD experiment for one dataset / model pair.

		Returns dict with keys:
				name, model_name, N, D, S, lam,
				theta_star, A, BBT, posterior_samples, mixing_times,
				variants: {
						"constant SGD":   {samples, sigma_empirical, sigma_theory, kl, eps},
						"constant SGD-d": {samples, sigma_empirical, sigma_theory, kl},
						"constant SGD-f": {samples, sigma_empirical, sigma_theory, kl, alpha},
				}
		"""
		np.random.seed(42)
		if cfg is None:
		...
		print(f"\n{'='*62}")
		print(f"  {dataset.name}  |  N={N}, D={D}, S={S}, "
					f"model={type(model).__name__}, λ={lam}")
		print(f"{'='*62}")
		...
		print(f"      L  at MAP = {lmap:.2e}")
		print(f"    |∇L| at MAP = {np.linalg.norm(gmap):.2e}")
		...
		print(f"    λ_min(A) = {eigs_A.min():.3e},  λ_max(A) = {eigs_A.max():.3e}")

		# ── BBT ────────────────────────────────────────────────────────────
		print(f"  Computing BB^T …")
		BBT = estimate_noise_cov(theta_star, dataset, model, S)
		print(f"    Tr(BB^T) = {np.trace(BBT):.4e}")
		eigs_BBT = np.linalg.eigvalsh(BBT)
		print(f"    λ_min(BB^T) = {eigs_BBT.min():.3e},  λ_max(BB^T) = {eigs_BBT.max():.3e}")
		...
		print(f"    τ_mix ∈ [{tau_fast:.1f}, {tau_slow:.1f}] steps")
		mixing_times = {"tau_slowest": tau_slow, "tau_fastest": tau_fast}

		sgd_kw = dict(
				dataset=dataset, model=model, S=S, lam=lam,
				n_runs=cfg.n_runs,
				n_steps_per_run=cfg.n_steps_per_run,
				n_samples_per_run=cfg.n_samples_per_run,
		)

		variants: dict[str, dict] = {}

		# ══ Constant SGD ══════════════════════════════════════════════════
		print("  [SGD]   running constant SGD …")
		eps1   = optimal_eps(S, model.nb_params, N, BBT)
		#alpha1 = eps1 / eps_raw
		#if alpha1 < 0.99:
		#		print(f"    ε clamped {eps_raw:.3e} → {eps1:.3e}  (α={alpha1:.4f})")
		...
		kld    = kl_divergence(empd if empd is not None else thd, A, N)
		print(f"    KL = {kld:.4f}")
		variants["constant SGD-d"] = dict(samples=sd, sigma_empirical=empd, sigma_theory=thd, kl=kld)

		# ══ Constant SGD-f ═════════════════════════════════════════════════
		print("  [SGD-f] running constant SGD-f …")
		...
		klf    = kl_divergence(empf if empf is not None else thf, A, N)
		print(f"    KL = {klf:.6f}")
		variants["constant SGD-f"] = dict(samples=sf, sigma_empirical=empf, sigma_theory=thf, kl=klf)

		return dict(
				name=dataset.name, model_name=type(model).__name__,
				N=N, D=D, S=S, lam=lam,
				theta_star=theta_star, A=A, BBT=BBT,
...
def explained_variance_threshold(
		eigs:        np.ndarray,   # eigenvalues, any order
		variance_explained: float = 0.99,
		squared: bool = False
) -> tuple[float, int]:
		"""
		Find the smallest threshold τ such that the eigenvalues above τ
		explain at least `variance_explained` fraction of the total.
		
		Returns (τ, effective_rank).
		"""
		eigs_sorted = np.sort(np.maximum(eigs, 0))[::-1] # descending
		if squared:
		...
		# smallest k such that top-k eigenvalues explain ≥ variance_explained
		k       = int(np.searchsorted(cumvar, variance_explained)) + 1
		k       = min(k, len(eigs_sorted))
		thresh  = eigs_sorted[k - 1]        # eigenvalue at the cutoff
		
		return float(thresh), k

# ══════════════════════════════════════════════════════════════════════
# TANGENT SUBSPACE PROJECTION
# Projects BBT onto range(J^T) where J is the output Jacobian.
#
# Key identity: g_n = J_n^T · (∂ℓ/∂f)(f_n, y_n)
# Therefore range(BBT) ⊆ range(J^T) exactly (no approximation).
# The tangent basis is model-aware and cleaner than SVD of gradient matrix G.
# ══════════════════════════════════════════════════════════════════════
...
def compute_jacobian(
    ...
    """
    Compute Jacobian J ∈ R^{m × P} of model outputs w.r.t. parameters.

    m = N × C  where C = output dimension per sample:
        C=1   for LinearModel, LogisticModel, NeuralNetModel(regression/binary)
        C=K   for NeuralNetModel(multiclass)

    For each sample n and output channel c:
        J[n*C + c, :] = ∂f_c(x_n; θ) / ∂θ

    The gradient noise lives in range(J^T) by the chain rule,
    so range(BBT) ⊆ range(J^T) exactly.
    ...
    if isinstance(model, LinearModel):
        return dataset.X.astype(np.float64)          # (N, P) = (N, D)

    # ── LogisticModel ─────────────────────────────────────────────────
    # f(x; θ) = σ(x^T θ),  ∂f/∂θ = σ'(x^T θ) · x = p(1-p) · x
    if isinstance(model, LogisticModel):
        p  = LogisticModel._sigmoid(dataset.X @ theta)   # (N,)
        w  = p * (1.0 - p)                               # σ'
        return (dataset.X * w[:, None]).astype(np.float64)   # (N, P)

    # ── NeuralNetModel ────────────────────────────────────────────────
    # Use torch.autograd row-by-row (output-dim × param sweeps).
    # For regression/binary: C=1, m=N.
    # For multiclass:        C=K, m=N*K.
    if isinstance(model, NeuralNetModel):
        import torch
        from torch.nn.utils.stateless import functional_call
        ...
        # Determine output dim C from a single forward pass
        ...
        for start in tqdm(range(0, N, batch), desc="Jacobian"):
            ...
            for i in range(b):
                ...
                th_t = torch.tensor(theta, dtype=torch.float32,
                                    requires_grad=False)
                # Jacobian of f (C-dim output) w.r.t. theta (P-dim)
    ...
    raise NotImplementedError(
        f"compute_jacobian not implemented for {type(model).__name__}")

def project_BBT_to_tangent(
    BBT:        np.ndarray,
    J:          np.ndarray,
    theta_star: np.ndarray = None,
    rank:       int        = None,     # force rank; None → from tol
    tol:        float      = 1e-8,     # relative singular value threshold
    verbose:    bool       = True,
) -> dict:
    """
    Project BBT onto the tangent subspace range(J^T).

    By the chain rule, every per-sample gradient satisfies:
        g_n = J_n^T · (∂ℓ/∂f)(f_n, y_n)
    so range(BBT) ⊆ range(J^T) exactly — no information is lost.

    Parameters
    ----------
    BBT        : (P, P) gradient noise covariance
    J          : (m, P) output Jacobian,  m = N*C
    theta_star : (P,) MAP estimate — projected to reduced coords if given
    rank       : force tangent rank; None → threshold at tol * σ_max
    tol        : relative singular value cutoff for automatic rank selection

    Returns
    -------
    dict with keys:
        V          (P, r)  orthonormal basis for range(J^T)
        BBT_red    (r, r)  V^T BBT V  — BBT in tangent coordinates
        BBT_proj   (P, P)  V V^T BBT V V^T  — projected back to param space
        r          int     tangent rank
        theta_red  (r,)    theta_star in tangent coords  (or None)
        singulars  (r,)    singular values of J used to build V
        leakage    float   fraction of Tr(BBT) outside tangent subspace
    """
    m, P = J.shape

    # ── Basis for range(J^T) via economy SVD ─────────────────────────
    # J = U Σ V^T  →  range(J^T) = span(columns of V with σ > threshold)
    if m >= P:
        # More outputs than params: direct SVD of J
        _, s, Vt = np.linalg.svd(J, full_matrices=False)   # s: (min(m,P),)
        V_full   = Vt.T                                      # (P, min(m,P))
    else:
        # Fewer outputs than params: dual trick — form (m×m) Gram K=JJ^T
        # nonzero singular values of J = sqrt(eigenvalues of K)
        K               = J @ J.T                           # (m, m)
        eigvals, eigvecs = np.linalg.eigh(K)               # ascending
        eigvals         = eigvals[::-1]                     # descending
        eigvecs         = eigvecs[:, ::-1]
        s               = np.sqrt(np.maximum(eigvals, 0.0))
        # V = J^T U Σ^{-1}  (thin columns spanning range(J^T))
        nonzero  = s > tol * s[0]
        U_nz     = eigvecs[:, nonzero]                      # (m, r_nz)
        s_nz     = s[nonzero]
        V_full   = J.T @ (U_nz / s_nz[None, :])            # (P, r_nz)
        # re-orthonormalise for numerical safety
    ...
    V         = V_full[:, :r]         # (P, r)  orthonormal
    singulars = s[:r]

    # ── Project BBT ───────────────────────────────────────────────────
    VtB      = V.T @ BBT              # (r, P)
    BBT_red  = VtB @ V               # (r, r)  = V^T BBT V
    BBT_proj = V @ BBT_red @ V.T     # (P, P)  rank-r projection

    # ── Leakage: how much of Tr(BBT) sits outside tangent subspace ────
    tr_total   = np.trace(BBT)
    tr_tangent = np.trace(BBT_red)
    leakage    = 1.0 - tr_tangent / max(tr_total, 1e-30)

    # ── Projected MAP coords ──────────────────────────────────────────
    theta_red = V.T @ theta_star if theta_star is not None else None

    if verbose:
        eigs_red  = np.linalg.eigvalsh(BBT_red)
        eigs_full = np.linalg.eigvalsh(BBT)
        print(f"\n  Tangent subspace projection:")
        print(f"    J shape             : {J.shape}  (m={m}, P={P})")
        print(f"    Tangent rank r      : {r} / {P}  "
              f"({100*r/P:.1f}% of param space)")
        print(f"    σ_max(J)            : {singulars[0]:.3e}")
        print(f"    σ_min(J, kept)      : {singulars[-1]:.3e}")
        print(f"    Tr(BBT)             : {tr_total:.4e}")
        print(f"    Tr(BBT in tangent)  : {tr_tangent:.4e}  "
              f"({100*(1-leakage):.2f}% captured)")
        print(f"    Leakage             : {100*leakage:.4f}%  "
              f"(should be ≈ 0)")
        print(f"    BBT_red λ range     : [{eigs_red.max():.3e}, "   # max first (descending view)
              f"{eigs_red[eigs_red>0].min():.3e}]")
        if leakage > 0.01:
            print(f"    !! Leakage > 1% — J may be computed at wrong θ "
                  f"or model is stochastic")
...
def run_sgd_tangent(
		theta_star: np.ndarray,
		V:          np.ndarray,      # (P, r) tangent basis
		A_tan:      np.ndarray,      # (r, r) — for OU theory only, not used in SGD
		BBT_tan:    np.ndarray,      # (r, r) — for OU theory only
		dataset:    Dataset,
		model:      ModelInterface,
		S:          int,
		lam:        float,
		eps:        float      = None,
		H_tan:      np.ndarray = None,   # (r, r) preconditioner in tangent coords
		n_runs:            int = 1,
		n_steps_per_run:   int = 50_000,
		n_samples_per_run: int = 20_000,
		...
		"""
		Run SGD in the r-dimensional tangent subspace.

		Reparametrisation: θ = θ* + V φ,  φ ∈ R^r
		Gradient:          ∇_φ L = V^T ∇_θ L(θ* + V φ)
		Step:              φ ← φ - ε · ∇_φ L   (or H_tan @ ∇_φ L)

		Samples returned in tangent coordinates, centred at 0 (i.e. φ = V^T(θ - θ*)).
		"""
		N    = len(dataset.y)
		r    = V.shape[1]
		burn = n_steps_per_run - n_samples_per_run
		...
		for run in tqdm(range(n_runs), desc="Runs"):
				phi     = np.zeros(r)           # start at θ* in tangent coords
				...
				pbar = tqdm(range(n_steps_per_run), desc="Steps", position=1, leave=False)
				for step in pbar:
						# Reconstruct full-space θ for gradient computation
						theta  = theta_star + V @ phi

						# Mini-batch gradient in full space
						idx    = np.random.randint(0, N, S)
						g_full = model.stoch_grad(theta, dataset.X[idx], dataset.y[idx],
																			N, lam)
...
def _pick_pc_directions(post_cov: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
		"""Smallest and largest eigenvectors of the posterior covariance."""
		vals, vecs = np.linalg.eigh(post_cov)
		return vecs[:, -1], vecs[:, 0]   # largest PC, smallest PC


def _draw_panel(
		ax, theta_star, post_samples, sgd_samples,
		sigma_theory, sigma_empirical, post_cov,
		dims, n_std, max_scatter, title,
):
		...
		ax.scatter(ps[rng.choice(len(ps), min(max_scatter, len(ps)), replace=False), 0],
							 ...
							 c="#3355dd", marker="x", s=12, alpha=0.35, linewidths=0.7,
							 zorder=2, label="posterior")

		# SGD iterates (cyan +) or Lyapunov draws
		label = "iterates"
...
def plot_figure1(
		...
		methods: tuple[str, ...] = ("constant SGD", "constant SGD-d", "constant SGD-f"),
		n_std: float = 2.0,
		max_scatter: int = 3_000,
		...
		for row, exp in enumerate(experiments):
				...
				for col, method in enumerate(methods):
						...
						_draw_panel(
								ax=axes[row][col],
								theta_star=exp["theta_star"],
								post_samples=exp["posterior_samples"],
								sgd_samples=var["samples"],
								sigma_theory=var["sigma_theory"],
								sigma_empirical=var.get("sigma_empirical"),
								post_cov=post_cov,
								dims=dims, n_std=n_std, max_scatter=max_scatter,
								title=f"{method}\n({exp['name']})",
						)
...
def plot_gradient_noise(
		...
		theta_star: np.ndarray,
		dataset:    Dataset,
		BBT_eigs:   np.ndarray,     # precomputed BB^T eigenvalues (length ≤ P)
		grad_mean:  np.ndarray,
		S:          int   = 100,
		n_samples:  int   = 50_000,
		n_pca:      int   = 5_000,  # subset for PCA (float32, P-wide)
		batch_size: int   = 512,    # batch size for reference norm generation
		save_path:  str   = None,
		figsize:    tuple = (11, 3.2),
		noise_norm_only: bool = False
) -> plt.Figure:
		"""
		4-panel gradient noise figure (memory-efficient).

		Memory budget:
			- Never allocates (n_samples × P) — only (n_pca × P) float32
			- Reference norms generated in batches of batch_size × P
			- Two streaming passes over gradients (each O(P) per step)

		Panels:
			0  PC1 histogram vs N(0,1)
			1  PC1 QQ-plot
			2  Norm histogram + χ reference (Gaussian BBT/S model)
			3  Log-log CCDF
		"""
		...
		# ══ Pass 1: centred norms + PCA subset ════════════════════════════
		# Stores: grad_norms (n_samples,) + grads_pca (n_pca × P, float32)
		...
		for i in tqdm(range(n_samples), desc="Sample gradient minibatches"):
				...
				gc   = g - grad_mean                       # centred gradient
		...
		if not noise_norm_only:
				...
				del grads_pca   # free (n_pca × P) float32

		# ══ Gaussian reference norms — batched ════════════════════════════
		# g_ref ~ N(0, BBT/S): g_ref = z * sqrt(eigs),  ||g_ref|| computed per batch
		...
		eigs_full    /= float(S)
		sqrt_eigs     = np.sqrt(eigs_full)             # (P,) float32
		...
		if not noise_norm_only:
				i = 2
				# ── Panel 0: PC1 histogram ────────────────────────────────────────
				ax = axes[0]
				ax.hist(pc1_n, bins=80, density=True, color=C["data"], alpha=0.35, edgecolor="none", label="Empirical")
				ax.plot(t_std, stats.norm.pdf(t_std), color=C["ref"], lw=1.5, ls="--", label=r"$\mathcal{N}(0,1)$")
				ax.set_title(f"Gradient-noise projection\n(PC 1, {pca.explained_variance_ratio_[0]:.1%} var.)", fontsize=8.5)
				ax.set_xlabel(r"$\Delta \widehat{g}_{\mathcal{S}}$ (on PC 1)", fontsize=8)
				...
				# ── Panel 1: PC1 QQ-plot ──────────────────────────────────────────
				ax = axes[1]
				(osm, osr), (slope, intercept, _) = stats.probplot(pc1_n, dist="norm")
		...
		# ── Panel 2: Norm histogram + χ reference ─────────────────────────
		ax   = axes[i]
		...
		if has_ref:
				ax.plot(t_n, gaussian_kde(norms_gauss, bw_method="scott")(t_n), color=C["ref"], lw=1.5, ls="--", label=r"$\chi$ ref.")
		ax.set_title("Gradient-noise norm", fontsize=8.5)
		ax.set_xlabel(r"$\|\Delta \widehat{g}_{\mathcal{S}}\|$", fontsize=8)
		ax.set_ylabel("Density", fontsize=8)
		ax.set_xlim(0, tmax)
		ax.legend(fontsize=7, framealpha=0.7, loc="upper right")
		ax.tick_params(labelsize=7)

		# ── Panel 3: Log-log CCDF ─────────────────────────────────────────
		ax    = axes[i + 1]
		...
		if has_ref:
				...
				rnk_g = np.arange(1, len(sn_g) + 1) / len(sn_g)
				ax.loglog(sn_g, rnk_g, color=C["ref"], lw=1.2, ls="--", label=r"$\chi$ ref.")
		ax.set_title("Log-log tail (CCDF)", fontsize=8.5)
		ax.set_xlabel(r"$\|\Delta \widehat{g}_{\mathcal{S}}\|$", fontsize=8)
		ax.set_ylabel(r"$\mathbb{P}(\|\Delta \widehat{g}_{\mathcal{S}}\| > t)$", fontsize=8)
		ax.legend(fontsize=7, framealpha=0.7)
		ax.tick_params(labelsize=7)
...
_PAPER_VALUES = {
		"constant SGD":   {"Wine": 18.7,  "Skin": 0.471, "Protein": 1000.9},
		"constant SGD-d": {"Wine": 14.0,  "Skin": 0.921, "Protein": 678.4},
		"constant SGD-f": {"Wine": 0.7,   "Skin": 0.005, "Protein": 1.8},
}
...
def print_table(experiments: list[dict]) -> None:
    ...
    # Footer: show which dimension was used
...
if __name__ == "__main__":
		...
		cfg = SGDConfig(
				n_runs=10,
				n_steps_per_run=5_000,
				n_samples_per_run=100,
				n_posterior=5_000,
		)

		# ── Main experiments: SGD variants vs posterior ─────────────────────
		print("\n" + "=" * 60 + "\n  Main experiments (constant SGD / SGD-d / SGD-f)\n" + "=" * 60)
		...
		print_table([
				exp_wine, exp_skin, exp_protein, exp_california, exp_adults,
		])
		plot_figure1(
				experiments=[exp_california, exp_adults],
				save_path="figure_california_income.pdf",
				dpi=300,
				methods=("constant SGD", "constant SGD-d", "constant SGD-f"),
		)

		# ── Gradient-noise diagnostics (norm histograms) ────────────────────
		...
		_d = [
				(ds_wine, LinearModel(), 100, 1.0),
				(ds_wine, LinearModel(bias=True), 100, 1.0),
				
				(ds_skin, LogisticModel(), 1_000, 1.0),
				(ds_skin, LogisticModel(bias=True), 1_000, 1.0),
				
				(ds_protein, LinearModel(), 100, 1.0),
				(ds_protein, LinearModel(bias=True), 100, 1.0),
				
				(ds_california, LinearModel(), 100, 1.0),
				(ds_california, LinearModel(bias=True), 100, 1.0),
				
				(ds_adults, LogisticModel(), 100, 1.0),
				(ds_adults, LogisticModel(bias=True), 100, 1.0),
		...
		for ds, model, S, lbd in _d:
				if isinstance(model, NeuralNetModel):
						...
						summary(model._net, input_size=(1, ds.X.shape[1]))
				else:
						model.nb_params = ds.X.shape[1] if not hasattr(model, 'bias') else (ds.X.shape[1] + model.bias)
				print(f"Dataset: {ds.name}(N={ds.X.shape[0]}, D={ds.X.shape[1]}), Model: {model.__class__.__name__}(D={model.nb_params})")
